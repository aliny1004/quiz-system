...
│   │   │   ├── 手機考試中三條線題目面板開關
│   │   │   ├── 明暗模式切換器與使用者頭像，手機考試頁隱藏
│   │   │   └── 作答計時器 #examTimer，首頁隱藏、測驗中顯示，手機考試頁保留單排
│   │   │
│   │   ├── #accessSection
...
- 關閉填空題輸入框的瀏覽器自動完成、輸入紀錄提示、自動修正與拼字檢查

### v14.0.4

- 修正新使用者或新裝置快速交卷時，Supabase 題庫尚未完成背景同步而導致學習紀錄外鍵寫入失敗的問題
- 補強統計與錯題同步前的題庫存在檢查，避免雲端紀錄漏寫

### v14.0.3

- 修正多頁拆分後，考試頁讀不到首頁測驗設定導致答案提示等模式失效的問題
- 修正結算頁「僅錯題」再次測驗在多頁考試頁可能碰到首頁 DOM 而失效的問題
- 回歸檢查答案提示、出題範圍、隱藏計時器、統計頁導頁與再次測驗流程

### v14.0.2

- 調整結算頁 header / footer 行為，使其與考試頁一致，可依滑動方向收合與展開

### v14.0.1

- 修正完成作答後進入結算畫面時，header / footer 不應自動收合的問題
- 修正結算畫面被延遲自動捲動影響，導致標題被 header 遮住的問題
- 修正考試頁、結算頁與學習統計頁的瀏覽器分頁標題
...
- 修正考試頁 header 收合時內容區突然彈動的問題
- 考試頁 header 改為依捲動距離逐步縮小，桌面與手機都會平滑收起

### v13.3.4

- 調整桌面版測驗模式排序：答案提示移至第一排中間，隱藏已答對題目移至第一排右側，錯題模式移至第二排中間
- 調整手機版測驗模式排序：答案提示移至錯題模式上方

### v13.3.3

- 修正手機考試頁 header 收合時造成內容跳動的問題
- 手機考試頁 header 隱藏明暗模式開關與使用者頭像，只保留題目面板開關、標題與作答時間
- 壓縮手機考試頁 header 高度，避免窄螢幕變成雙排
