...
- 支援使用者名稱訪問頁，使用者名稱會寫入 Supabase，供後續資料分流
- 自動分辨選擇題與填空題
- 歷史題庫保存在 LocalStorage，並可依使用者背景同步至 Supabase
- 支援打亂題目、打亂選項、隱藏已答對題目
...
- 支援自動捲動到下一題
- 支援本次錯題再次測驗
- 支援學習統計：正確率歷史、每題作答次數、答對率、最後作答日期，並可依使用者同步至 Supabase
- 支援啟動時從 Supabase 回補題庫、統計、錯題紀錄與答對題目到 LocalStorage
- 支援手機與桌面自適應排版
